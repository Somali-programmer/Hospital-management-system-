...
export default function Reports() {
  const { billing, patients, staff, appointments } = useData();
...
const COLORS = ['#0ea5e9', '#6366f1', '#8b5cf6', '#ec4899'];
  ...
  return (
    <div className="space-y-12 max-w-7xl mx-auto h-full pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 px-2">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase leading-tight">Insight & Analytics</h2>
          <p className="text-sm text-slate-500 font-semibold uppercase tracking-wider mt-1">Operational Reports (Warbixino)</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={REVENUE_DATA}>
                ...
                <Tooltip 
                  contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 40px rgba(0,0,0,0.1)', fontWeight: 'bold', textTransform: 'uppercase', fontSize: '10px' }}
                />
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-[0_8px_40px_rgb(0,0,0,0.02)] space-y-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black uppercase text-slate-400">Average: 42</span>
            </div>
          </div>
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-[0_8px_40px_rgb(0,0,0,0.02)] space-y-8 lg:col-span-1">
          <h4 className="font-black text-slate-900 text-lg uppercase tracking-tight mb-8">Patient Demographic (Specialties)</h4>
          ...
          <div className="grid grid-cols-2 gap-4">
             {SPECIALTY_DISTRIBUTION.map((s, i) => (
                <div key={i} className="flex items-center gap-3">
                   ...
                   <p className="text-[10px] font-black uppercase text-slate-500 tracking-tight truncate leading-tight group-hover:text-slate-900">{s.name}</p>
                </div>
             ))}
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-[3.5rem] p-10 text-white relative overflow-hidden flex flex-col justify-center">
           ...
           <h3 className="text-3xl font-black uppercase tracking-tighter mb-4 relative z-10">Diagnostic Efficiency</h3>
           ...
           <div className="w-full bg-slate-800 h-2.5 rounded-full overflow-hidden relative z-10 mb-2">
             <div className="h-full bg-primary-500 w-[88%] rounded-full shadow-[0_0_15px_rgba(14,165,233,0.5)] transition-all duration-1000"></div>
           </div>
           <div className="flex justify-between items-center relative z-10">
              <span className="text-[10px] font-black uppercase text-slate-500">Current Target (95%)</span>
              <span className="text-[10px] font-black uppercase text-primary-400 font-mono tracking-widest">88% Achieved</span>
           </div>
        </div>
      </div>
    </div>
